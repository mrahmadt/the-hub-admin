                            ...
                            <table class="table table-hover table-listing">
                                <thead>
                                    <tr>
                                        ...
                                        <th is='sortable' :column="'name'">{{ trans('admin.application.columns.name') }}</th>
                                        <th is='sortable' :column="'category_id'">{{ trans('admin.application.columns.category_id') }}</th>

                                        <th is='sortable' :column="'activated'">{{ trans('admin.application.columns.activated') }}</th>
                                        <th is='sortable' :column="'isFeatured'">{{ trans('admin.application.columns.isFeatured') }}</th>
                                        <th></th>
                                    </tr>
                                ...
                                <tbody>
                                    <tr v-for="(item, index) in collection" :key="item.id" :class="bulkItems[item.id] ? 'bg-bulk' : ''">
                                        ...
                                        <img :src="item.icon" class="rounded" style="max-width:64px; max-hight:64px;"> @{{ item.name }}</td>
                                        <td v-else>@{{ item.name }}</td>
                                        <td>@{{ item.category.name }}</td>
                                        <td v-if="item.isFeatured">{{ trans('admin.yes') }}</td>
                                        <td v-else>{{ trans('admin.no') }}</td>
                                        <td v-if="item.activated">{{ trans('admin.yes') }}</td>
                                        <td v-else>{{ trans('admin.no') }}</td>
                                        <td>
